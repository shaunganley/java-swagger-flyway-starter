...
class JobRoleServicesTest {
    ...
    public void getAllJobRoles_shouldReturnListOfJobRoles() throws SQLException {
        ...
        jobRoles.add(
                new JobRole(2,"test2", "Belfast", "testCapability2", "testBand2", Date.valueOf("2000-10-11"))
        );
        Mockito.when(jobRoleDao.getAllJobRoles()).thenReturn(jobRoles);

        List<JobRoleResponse> expected = new ArrayList<>();
        expected.add(new JobRoleResponse("test", "Belfast","testCapability", "testBand", Date.valueOf("2000-10-10"))
        );
        expected.add(new JobRoleResponse("test2", "Belfast","testCapability2", "testBand2", Date.valueOf("2000-10-11"))
        );
    }
