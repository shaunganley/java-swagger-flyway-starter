...
public class JobRoleDao {
    private static final Logger LOGGER = Logger.getLogger(JobRoleDao.class.getName());

    public List<JobRole> getAllJobRoles(final Connection c) throws SQLException, DatabaseConnectionException {
    ...
    public JobRoleInfo getJobRoleById(final int id, final Connection c) throws SQLException, DatabaseConnectionException {
        String query = "SELECT "
                + "jr.id, jr.role_name AS RoleName, "
                + "l.name AS Location, "
                + "c.name AS Capability, "
        ...
        try (PreparedStatement statement = c.prepareStatement(query)) {
            ...
            LOGGER.log(Level.SEVERE, "Error fetching job role by id: " + id, e);
            throw e;
        }
